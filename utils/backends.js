
// Allows us to import in both HTML and NodeJS
if (typeof exports == "undefined") {
    exports = this;
}

if(typeof parse === 'undefined'){
    var parser = require('./parser.js');
    var parse = parser.parse;
}

class IBackend {
    constructor() {
        this.code = "";
        this.typeMap = {};
    }

    comment(txt) {
        this.code += "// " + txt + "\n";
    }

    genRetType = type => {
        if (type in this.typeMap) {
            return this.typeMap[type];
        }

        return undefined;
    }

    clear = () => this.code = "";

    getTransposeFunc = () => 'transpose';

    // @param fileName string for debugging, may be null
    genCode(txt, fileName) {
    }

    genVar(dstType, name, value) {
    }

    genDrawPointWS(posVarName, colorVarName) {
    }

    genFloat3x3Columns = (dstType, dstName, column0, column1, column2) => {

    }



    genTranspose = (dstType, dstName, srcName) => {
        this.code += this.genRetType(dstType);
        this.code += ` ${dstName} = ${this.getTransposeFunc()}(${srcName});\n`
    }

    genNormalize = (dstType, dstName, srcName) => {

    }

    genGetFloat(dstName, srcName, component) {
        this.code += "float " + dstName + " = " + srcName + "." + component + ";\n";
    }

    genMatrixMul(dstType, dstName, src1, src2) { }

    genPrintVar(srcName) { }

    genMatrixGetRowColumn(dstType, dstName, srcName, row, column) { }

    genMatrixSetRowColumn(varName, row, column, value) { }

    translateName = type => this.genRetType(type);
}

class BackendGLM extends IBackend {
    constructor() {
        super();
        this.typeMap = {
            "float2x2": "glm::mat2",
            "float3x3": "glm::mat3",
            "float4x4": "glm::mat4",
            "float": "float",
            "float2": "glm::vec2",
            "float3": "glm::vec3",
            "float4": "glm::vec4",
            "int2": "glm::ivec2",
            "int3": "glm::ivec3",
            "int4": "glm::ivec4",
            "uint2": "glm::uvec2",
            "uint3": "glm::uvec3",
            "uint4": "glm::uvec4",
            "quat": "glm::quat"
        };
    }

    genCode(txt, fileName) {
    }

    genVar(dstType, name, value) {
        this.code += "var" + " " + name + " = " + genRetType(dstType, "glm-js") + "(" + value + ");\n";
    }
}


class BackendGLMJS extends IBackend {
    constructor() {
        super();
        this.typeMap = {
            "float2x2": "glm.mat2",
            "float3x3": "glm.mat3",
            "float4x4": "glm.mat4",
            "float": "float",
            "float2": "glm.vec2",
            "float3": "glm.vec3",
            "float4": "glm.vec4",
            "int2": "glm.ivec2",
            "int3": "glm.ivec3",
            "int4": "glm.ivec4",
            "uint2": "glm.uvec2",
            "uint3": "glm.uvec3",
            "uint4": "glm.uvec4",
            "quat": "glm.quat"
        };
    }

    genCode(txt, fileName) {
    }

    genVar(dstType, name, value) {
        this.code += "var" + " " + name + " = " + this.genRetType(dstType) + "(" + value + ");\n";
    }
}

class BackendPassthrough extends IBackend {
    constructor() {
        super();
    }
    // Default backend is HLSL, so return as is.
    translateName = type => type;
    genRetType = type => type;

    genCode(txt, fileName) {
        console.assert(txt !== undefined);
        this.code = "";
        this.code += "// generated by BackendPassthrough\n";
        this.code += parse(txt, new BackendPassthrough(), fileName);
    }

    genVar(dstType, name, value) {
        var type = this.genRetType(dstType);

        this.code += type;

        if (value)
            this.code += " " + name + " = " + type + "(" + value + ");\n";
        else
            this.code += " " + name + ";\n";
    }
}

class BackendGLSL extends IBackend {
    constructor() {
        super();
        this.typeMap = {
            "float2x2": "mat2",
            "float3x3": "mat3",
            "float4x4": "mat4",
            "float": "float",
            "float2": "vec2",
            "float3": "vec3",
            "float4": "vec4",
            "int2": "ivec2",
            "int3": "ivec3",
            "int4": "ivec4",
            "uint2": "uvec2",
            "uint3": "uvec3",
            "uint4": "uvec4",
            "frac": "fract",
            "lerp": "mix",
            "rsqrt": "inversesqrt"
        };
    }

    genCode(txt, fileName) {
        console.assert(txt !== undefined);
        this.code += "// generated by BackendGLSL\n";
        this.code += parse(txt, new BackendGLSL(), fileName);
    }

    genVar(dstType, name, value) {
        var type = this.genRetType(dstType);

        this.code += type;

        if (value)
            this.code += " " + name + " = " + type + "(" + value + ");\n";
        else
            this.code += " " + name + ";\n";
    }

    genDrawPointWS(posVarName, colorVarName) {
        this.code += 'drawPointWS(context, ' + posVarName + ', ' + colorVarName + ');\n';
    }
}

// like Backend_GLSL but expands #include
class BackendWebGL extends BackendGLSL {
    constructor() {
        super();
    }
    genCode(txt, fileName) {
        console.assert(txt !== undefined);
        let backend = new BackendWebGL();
        backend.expandIncludes = true;
        this.code = "";
        this.code += "// generated by BackendWebGL\n";
        this.code += parse(txt, backend, fileName);
    }
}

// available backends to be usable by s2h.getBackend()
const gBackends = {
    'hlsl': new BackendPassthrough(),
    'glsl': new BackendGLSL(),
    'glm': new BackendGLM(),
    'webgl': new BackendWebGL(),
    'glm-js': new BackendGLMJS(),
    'none': new IBackend()
};

// stores a dictionary of backends for different code types
const s2h = {
    getBackend: (codeType) => {
        codeType = codeType.toLowerCase();
        if (codeType in gBackends) {
            return gBackends[codeType];
        }

        const nonBackend = 'none';
        return gBackends[nonBackend];
    },
    getBackendList: () => Object.keys(gBackends)
};

exports.s2h = s2h;